    ...
    while True:
        try:
            ...
            if lower_limit <= number <= upper_limit:
                ...
                logging.error(
                    f'Номер должен быть между {lower_limit} и {upper_limit}')
        except ValueError:
            logging.error('Неправильный ввод, введите целое число.')
...
def main(lower_limit: int, upper_limit: int, number: int = None):
    if number is None:
        message_for_user = f'Введите целое число между {lower_limit} и {upper_limit}: '
